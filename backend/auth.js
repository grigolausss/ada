...
const createSignedToken = (sessionId, duration = 30 * 60 * 1000) => {
    const payload = { sessionId, expires: Date.now() + duration };
...
const verifySignedToken = (token) => {
    ...
    if (!crypto.timingSafeEqual(Buffer.from(signature), Buffer.from(expectedSignature))) return { valid: false };
    const payload = JSON.parse(Buffer.from(payloadBase64, 'base64url').toString('utf8'));
    if (Date.now() > payload.expires) return { valid: false };
...
module.exports = { createSignedToken, verifySignedToken, createOtp, hashValue };
